...
*/
	...
	if(tid<re_size && bid < KER_NUM)
	{
		...
		copy_data_to_share(train,train_tmp,tid,offset,head,x*y*z);
		...
		for(int i=0;i<x*y*P_NUM;i++){
			mid = mid + kernel_tmp[0][i] * train_tmp[0][tid*x*y*LEAP + i];
		}
		mid = mid + bias[bid];
...
__global__ static void bp_update_kernel(int iter,int i0, double LEARN_RATE, int x, int y, int z, int mre_num,int re_size, int * mre_index, double * delta_22, double * data, double * kernel,double * bias0)
{
	...
	if(tid < ((NEIGHBOR+1)*P_NUM) && (bid < KER_NUM))
	{
		//int head = i0*x*y*z;
		//int length = x*y*z;
		//int offset = (length - 1)/(x*y*P_NUM) + 1;
		//__shared__ double train_tmp[1][MAX_MRE];
		...
		kernel[tid + bid*x*y*P_NUM] = kernel[tid + bid*x*y*P_NUM] - LEARN_RATE*delta_k_w[0][tid];
   ...
   //fprintf(stdout,"processed valid data:%lf %lf %lf %lf\n",processed_valid[0],processed_valid[1],processed_valid[2],processed_valid[3]);
	...
	for(int j=0; j<300; j++){
		...
		for(int i0=0; i0<train_size; i0++){
			...
			convol<<<KER_NUM,re_size/*,(NEIGHBOR+1)*z*sizeof(double)*/>>>(iter,i0,gpu_processed_train,gpu_kernel,gpu_re,gpu_bias0,3,3,z,re_size);
			cudaDeviceSynchronize();	
			...
			cudaDeviceSynchronize();
            		//反向传播，全连接层
			bp_fullconnect<<<1,NEU_NUM1/*,NEU_NUM2*/>>>(iter,LEARN_RATE,gpu_omega2,gpu_bias1,gpu_F1,gpu_delta_Lz,gpu_delta_fa,gpu_delta_fz);
			cudaDeviceSynchronize();
            		//反向传播，maxpooling层
			...
			cudaDeviceSynchronize();
            		//反向传播，map到卷积层
			bp_update_kernel<<<KER_NUM,(NEIGHBOR+1)*P_NUM,(NEIGHBOR+1)*z*sizeof(double)>>>(iter,i0,LEARN_RATE,3,3,z,mre_num,re_size,gpu_mre_index,gpu_delta_22,gpu_processed_train,gpu_kernel,gpu_bias0);

			cudaDeviceSynchronize();

			//if(i0<10){
			//	SAFE_CALL(cudaMemcpy(bias0, gpu_bias0, sizeof(double) * KER_NUM, cudaMemcpyDeviceToHost));
			//	fprintf(stdout,"gpu_bias0:%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",bias0[0],bias0[1],bias0[2],bias0[3],bias0[4],bias0[5],bias0[6],bias0[7],bias0[8],bias0[9],bias0[10],bias0[11],bias0[12]);
			//	fprintf(stdout,"Sum of O2:%lf\b",O2[0]+O2[1]+O2[2]+O2[3]+O2[4]+O2[5]+O2[6]+O2[7]+O2[8]+O2[9]+O2[10]+O2[11]+O2[12]);
			//	SAFE_CALL(cudaMemcpy(delta_22, gpu_delta_22, sizeof(double) * re_size * KER_NUM, cudaMemcpyDeviceToHost));
			//	fprintf(stdout,"gpu_delta_22:%lf %lf %lf %lf\n",delta_22[0], delta_22[1], delta_22[2], delta_22[3]);
			//}
		...
		fprintf(stdout,"The %dth iteration. Current loss:%lf\n",j,single_rate);
        	/*if(single_rate > 0.9){
            		break;
        	}*/
		...
		double new_min = min(correct_rate);
        	...
        	if(count >= VALID_BATCH && (LEARN_RATE>=0.001) && (single_rate - cur_min < 0.002)) {
		...
		if(count >= VALID_BATCH && (cur_max - single_rate < 0.3) && (LEARN_RATE>=0.002)){
			LEARN_RATE = LEARN_RATE / 2;
	...
	for (int i1=0; i1<test_size; i1++){
		int iter = 0;
		convol<<<KER_NUM,re_size/*,(NEIGHBOR+1)*z*sizeof(double)*/>>>(iter,i1,gpu_processed_test,gpu_kernel,gpu_re,gpu_bias0,3,3,z,re_size);
		cudaDeviceSynchronize();
