...
#    model.add(Reshape((row_of_reshape,int(number_of_con_filters))))
    ...
    number_of_con_filters_second = 26
    second_convolutional_kernel_size = 5
    ...
    print("经过第三次卷积以后，卷积出来的feature map大小为：" + str(layer_con_3.output_shape))
    model.add(Activation('tanh'))
    
    layer_maxpooling4 = MaxPooling2D(pool_size = (max_pooling_kernel_size,max_pooling_kernel_size), strides=(2,2), border_mode='valid',dim_ordering='th')
    model.add(layer_maxpooling4)
    
    print("经过最后一次下采样之后，得到的feature map每一个大小为：" + str(layer_maxpooling4.output_shape))
    model.add(Flatten())
...
if __name__ == '__main__':
    run_network("/home/para/gitstore/HSICNN/hypertector/experiments/PU_1_1_2017_5_11_13_10/PU_1_1", 6, 1, 2, 100, 50, 0.0001, 0.0000001, 1000)
